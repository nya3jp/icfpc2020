...
fn machine_generated_heat(m: &Machine, heat: usize) -> Machine {
    Machine { generated_heat: m.generated_heat + heat, .. *m }
}
                        ...
                        if m.machine_id != (*shipnum as isize) {
                            ...
                            let newmachine = 
                                Machine {
                                    ...
                                    params: Param { energy: newmachine.params.energy - THRUST_ENERGY,
                                                    .. newmachine.params },
                                    .. newmachine
                                };
                            newstate.machines[i] = (newmachine, Some(ActionResult::Thruster { a: *delta }));
                        }
                    };
                    ncount += 1
            },
...
fn machine_update_coordinate(m: &Machine) -> Machine {
    Machine { position: m.position + m.velocity, .. *m }
}

fn machine_damage(m: &Machine, damage: usize) -> Machine {
    Machine { attack_heat: m.attack_heat + damage, .. *m}
}
